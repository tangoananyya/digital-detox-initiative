...
const PAYMENT_LINK = 'https://buy.stripe.com/eVqcN56FG4tBcKo3O6dfG04';
...
const REVIEWS = [
  { name: 'Yemi A.',      rating: 5, text: 'i stand all day at work and my calves were absolutely destroyed every evening. this thing changed my life i am not even joking. i use it while watching tv and wake up with zero soreness' },
  { name: 'Christine B.', rating: 4, text: 'Really well made and works exactly as described. I use it on my shoulders and the relief is immediate. Would give 5 stars but I wish the balls were a tiny bit firmer.' },
  { name: 'Jake T.',      rating: 5, text: 'bought this for leg recovery after runs and honestly it is better than my foam roller. the targeted pressure is way more effective and it is so much easier to use' },
  { name: 'Fatima N.',    rating: 5, text: 'ok this is genuinely one of the best purchases i have made this year?? my whole family uses it now. my mum uses it on her feet every night and says it is the only thing that helps her arthritis' },
  { name: 'Leo S.',       rating: 4, text: 'solid product. looks premium, feels premium. i use it at my desk between meetings and it keeps my shoulders from completely seizing up. good value for the price' },
  { name: 'Rachel K.',    rating: 5, text: 'I am a nurse and I am on my feet for 12 hour shifts. This is the first thing I reach for when I get home. My calves and ankles feel completely different after 5 minutes.' },
  { name: 'Marcus D.',    rating: 5, text: 'works great on the calves and the back of the neck. compact enough to keep in my laptop bag. have already recommended it to three people at the office' },
  { name: 'Sonia P.',     rating: 4, text: 'nice quality and it does the job. the teal colour is really pretty too. shipping was fast and packaging was minimal which i appreciated. would buy again' },
];
...
const HERO_IMG  = `${BASE}images/ballm2.png`;
const THUMBS    = ['ballm1', 'ballm2', 'ballm3'].map(n => `${BASE}images/${n}.png`);
...
export default function BallMassager() {
  ...
  useEffect(() => {
    const t = setInterval(() => setViewerCount(Math.floor(Math.random() * 13) + 29), 8000);
    return () => clearInterval(t);
  }, []);
  ...
  const addToCart = () => {
    const id = 'ball-massager';
    setCart(prev => {
      const existing = prev.find(i => i.id === id);
      if (existing) return prev.map(i => i.id === id ? { ...i, qty: i.qty + qty } : i);
      return [...prev, { id, name: 'Acupressure Roller Ball Massager', color: '', price: 24.99, qty }];
    });
    setCartOpen(true);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: P.linen, color: P.charcoal, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <section className="pp-hero" ref={heroRef}>
        <div>
          <div className="pp-hero-wrap" onMouseMove={handleZoomMove} onMouseLeave={handleZoomLeave} style={{ cursor: zoom.active ? 'none' : 'default' }}>
            <img src={heroImg} alt="Acupressure Roller Ball Massager" className="pp-hero-img" />
          </div>
        ...
        <div>
          ...
          <h1 className="pp-headline">
            Acupressure Roller<br />
            <span style={{ fontWeight: 600 }}>Ball Massager</span>
          </h1>
          <StarRow rating={4.6} count={3104} />
          ...
          <div className="pp-stock-row">
            ...
            <span className="pp-stock-text">Only 14 left in stock</span>
          </div>
          ...
          <ul className="pp-bullets">
            <li className="pp-bullet"><span className="pp-bullet-dot" />Releases deep muscle knots in shoulders, calves, thighs, and feet in minutes</li>
            <li className="pp-bullet"><span className="pp-bullet-dot" />Improves circulation and reduces post-screen fatigue with targeted acupressure</li>
            <li className="pp-bullet"><span className="pp-bullet-dot" />Portable and lightweight - use it at your desk, on the couch, or anywhere</li>
          </ul>
          ...
          <button className="pp-buy-now" onClick={() => goToCheckout('Acupressure Roller Ball Massager', 24.99 * qty)}>Buy Now</button>
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Designed for total muscle release</h2>
        <div className="pp-benefits-grid">
          {[
            { type: 'ball',    name: 'Dual-Track Ball Design',       desc: 'Six precision balls apply targeted myofascial pressure that adapts to any muscle group or body contour.' },
            { type: 'trigger', name: 'Instant Trigger Point Release', desc: 'Mimics professional deep tissue thumb pressure to dissolve knots in under 60 seconds per muscle.' },
            { type: 'spine',   name: 'Use It Anywhere',              desc: 'At your desk between calls, on the couch after work, or in bed before sleep. No setup, no noise, no power needed.' },
          ].map(b => (
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Precision Pressure. Total Release.</h2>
        <div className="pp-desc">
          <p>The Acupressure Roller Ball Massager uses six high-density acupressure balls mounted on a flexible U-shaped frame to deliver targeted myofascial release to any muscle group. The dual-track ball design allows both rolling and static pressure application, adapting to the natural contours of your calves, shoulders, thighs, and feet without requiring any awkward positioning.</p>
          <p>Unlike foam rollers that apply broad unfocused pressure, each individual ball targets a specific trigger point, mimicking the thumb pressure technique used in professional deep tissue massage. Roll slowly over a muscle belly for 60 seconds and feel the tension dissolve in real time.</p>
          <p>The frame is constructed from aerospace-grade ABS plastic that flexes without cracking and maintains its tension after thousands of uses. The balls are removable and wipe clean in seconds. Light enough to fit in a handbag, durable enough to last years.</p>
        </div>
        <div className="pp-specs">
          {[
            { label: 'Material',    value: 'ABS Frame, Acupressure Balls' },
            { label: 'Ball Count',  value: '6 Dual-Track Balls' },
            { label: 'Dimensions',  value: '32cm × 22cm' },
            { label: 'Weight',      value: '180g' },
            { label: 'Usage',       value: 'Calves, Shoulders, Thighs, Feet, Arms' },
            { label: 'Care',        value: 'Wipe Clean, Removable Balls' },
          ].map(s => (
      ...
      <section className="pp-cta-section pp-reveal">
        ...
        <h2 className="pp-cta-title">Your muscles remember every hour you spent at a screen.</h2>
        <p className="pp-cta-sub">Give them the release they have been waiting for. Free shipping. 30-day guarantee.</p>
        <button className="pp-cta-btn" onClick={() => goToCheckout('Acupressure Roller Ball Massager', 24.99)}>
          Get Yours Now - $24.99
        </button>
      </section>
      ...
      <div className={`pp-sticky-bar${showStickyBar ? ' pp-sticky-bar--visible' : ''}`} aria-hidden={!showStickyBar}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="pp-sticky-name">Acupressure Roller Ball Massager</div>
          <div className="pp-sticky-meta">$24.99</div>
        </div>
