  ...
  return (
    <div onClick={handleDismiss} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: visible ? 1 : 0, transition: 'opacity 0.35s ease' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#252018', borderRadius: 20, padding: '32px 28px 28px', maxWidth: 440, width: 'calc(100% - 40px)', boxShadow: '0 16px 60px rgba(0,0,0,0.5)', border: `1px solid ${P.border}`, transform: `translateY(${visible ? '0' : '24px'}) scale(${visible ? 1 : 0.97})`, transition: 'transform 0.38s cubic-bezier(.32,.72,0,1)' }}>
        ...
        <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 700, color: P.charcoal, textAlign: 'center', margin: '0 0 10px', lineHeight: 1.25 }}>
          Spending too much time on screens?
        </h3>
        <p style={{ fontSize: 14.5, lineHeight: 1.65, color: P.charcoalSoft, textAlign: 'center', margin: '0 0 24px' }}>
          Take our 60-second assessment to learn what screen time is doing to your body and how to reverse it.
        </p>
        <Link to="/quiz" style={{ display: 'block', width: '100%', padding: '15px 18px', background: P.brass, color: '#fff', borderRadius: 13, fontFamily: "'Inter', sans-serif", fontSize: 15, fontWeight: 600, textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box', marginBottom: 10 }}>
          Take the Free Assessment
        </Link>
        <button onClick={handleDismiss} style={{ display: 'block', width: '100%', padding: '12px 18px', background: 'transparent', border: 'none', fontFamily: "'Inter', sans-serif", fontSize: 14, color: P.charcoalSoft, cursor: 'pointer' }}>
          No thanks
        </button>
      </div>
    </div>
...
const PAYMENT_LINK = 'https://buy.stripe.com/14AeVdggg4tB8u81FYdfG02';
...
const REVIEWS = [
  { name: 'Carmen L.', rating: 5, text: 'i have scoliosis and have tried everything. this is the first thing that has actually given me consistent relief. i use it twice a day now and my physio is genuinely impressed' },
  { name: 'Paul M.',   rating: 5, text: 'Retired electrician, spent 30 years hunched in tight spaces. My lower back was basically destroyed. Three weeks with this and I am sleeping through the night for the first time in years.' },
  { name: 'Jasmine T.', rating: 4, text: 'ok so it is a little intense at first?? like the first few times i thought i was going to die lol but now i am obsessed. my back feels so much better and i stand way straighter' },
  { name: 'Rob K.',    rating: 5, text: 'Simple, effective, no nonsense. I have bought three of these now, one for home, one for the office, one for my mum. Does exactly what it says.' },
  { name: 'Aisha B.',  rating: 4, text: 'Really good product. The blush colour is beautiful and it looks nice enough to leave out in my living room. Only thing is I wish it came in a larger size for taller people.' },
  { name: 'Daniel W.', rating: 5, text: 'I am a personal trainer and I recommend this to every client who complains about lower back pain from sitting. It is the closest thing to a passive spinal adjustment you can do at home.' },
  { name: 'Mei C.',    rating: 5, text: 'bought this during a really bad flare up and honestly cried when i felt it work. the pressure on the nodules is intense but in the best way. nothing else has come close' },
  { name: 'Stuart F.', rating: 4, text: 'solid product. lavender colour is nicer in person than in photos. took about a week to get comfortable with the stretch level but now i look forward to it every evening' },
];
...
function ReviewCarousel({ reviews }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const v = mq.matches ? 1 : 3;
      setVisible(v);
      setCurrent(c => Math.min(c, reviews.length - v));
    };
    update();
                ...
                <div className="pp-review-card">
                  <div className="pp-review-stars">
                    {[...Array(r.rating)].map((_, j) => (
                      <svg key={j} width="14" height="14" viewBox="0 0 16 16" fill={P.brass}><polygon points="8,1.5 10,6 15,6.5 11.5,10 12.5,15 8,12.5 3.5,15 4.5,10 1,6.5 6,6" /></svg>
                    ))}
                  </div>
...
const ALL_THUMBS = ['back1', 'back2', 'back3', 'back4', 'back5'].map(n => `${BASE}images/${n}.png`);

const COLOR_CONFIG = {
  lavender: { hero: `${BASE}images/back4.png`, thumbs: ALL_THUMBS },
  blush:    { hero: `${BASE}images/back2.png`, thumbs: ALL_THUMBS },
  sky:      { hero: `${BASE}images/back3.png`, thumbs: ALL_THUMBS },
};
          ...
          {cart.length === 0 ? (
            ...
            <div key={item.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '18px 0', borderBottom: `1px solid ${P.border}` }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: P.charcoalSoft, marginBottom: 10 }}>{item.color}</div>
...
export default function BackStretcher() {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) { entry.target.classList.add('pp-revealed'); obs.unobserve(entry.target); }
      });
    }, { threshold: 0.1 });
  ...
  const addToCart = () => {
    ...
    const id = `back-stretcher::${color}`;
    setCart(prev => {
      const existing = prev.find(i => i.id === id);
      if (existing) return prev.map(i => i.id === id ? { ...i, qty: i.qty + qty } : i);
      return [...prev, { id, name: 'Spine Release Back Stretcher', color: colorLabel, price: 34.99, qty }];
    });
    setCartOpen(true);
  };
  ...
  const handleColorSelect = (key) => {
    setColor(key);
    setHeroImg(COLOR_CONFIG[key].hero);
  };
    ...
    <div style={{ minHeight: '100vh', background: P.linen, color: P.charcoal, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .pp-hero { max-width: 1800px; margin: 0 auto; padding: 64px 48px 56px; display: grid; grid-template-columns: 1.2fr 1fr; gap: 64px; align-items: start; }
        ...
        .pp-section { max-width: 1400px; margin: 0 auto; padding: 56px 48px; }
      ...
      <section className="pp-hero" ref={heroRef}>
        <div>
          <div className="pp-hero-wrap" onMouseMove={handleZoomMove} onMouseLeave={handleZoomLeave} style={{ cursor: zoom.active ? 'none' : 'default' }}>
            <img src={heroImg} alt="Spine Release Back Stretcher" className="pp-hero-img" />
          </div>
          <div className="pp-thumbs">
            {COLOR_CONFIG[color].thumbs.map((src) => (
        ...
        <div>
          ...
          <h1 className="pp-headline">
            Spine Release<br />
            <span style={{ fontWeight: 600 }}>Back Stretcher</span>
          </h1>
          <StarRow rating={4.7} count={1923} />
          ...
          <div className="pp-swatches">
            {[
              { key: 'lavender', label: 'Lavender', fill: '#C9B8E8' },
              { key: 'blush',    label: 'Blush',    fill: '#F0B8C0' },
              { key: 'sky',      label: 'Sky',      fill: '#B8D0E8' },
            ].map(s => (
              <button key={s.key} onClick={() => handleColorSelect(s.key)} className="pp-swatch-btn" aria-label={s.label} title={s.label} style={{ outline: color === s.key ? `2px solid ${P.brass}` : '2px solid transparent', outlineOffset: 2 }}>
                <span className="pp-swatch-circle" style={{ background: s.fill, border: '1px solid rgba(255,245,230,0.1)' }} />
          ...
          <div className="pp-stock-row">
            ...
            <span className="pp-stock-text">Only 8 left in stock</span>
          </div>

          <div className="pp-price-row">
            <span className="pp-price">$34.99</span>
            <span className="pp-price-strike">$69.99</span>
            <span className="pp-save">SAVE 50%</span>
          </div>

          <ul className="pp-bullets">
            <li className="pp-bullet"><span className="pp-bullet-dot" />Instant relief from back and neck pain with soothing acupressure in just 10 minutes a day</li>
            <li className="pp-bullet"><span className="pp-bullet-dot" />Helps heal chronic pain, sciatica, spinal stenosis, and herniated disc pain over time</li>
            <li className="pp-bullet"><span className="pp-bullet-dot" />Trusted by chiropractors, physical therapists, and personal trainers for daily use</li>
          </ul>
          ...
          <button className="pp-buy-now" onClick={() => goToCheckout('Spine Release Back Stretcher', 34.99 * qty)}>Buy Now</button>
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Designed for spinal recovery</h2>
        <div className="pp-benefits-grid">
          {[
            { type: 'spine',  name: 'Decompresses the Lumbar Spine', desc: 'Separates compressed vertebrae and restores natural disc spacing with passive gravitational traction.' },
            { type: 'nodule', name: 'Dual Acupressure Surface',       desc: 'Over 200 nodules target the erector spinae and multifidus to release deep muscle tension.' },
            { type: 'time',   name: 'Use Just 10 Min Daily',          desc: 'No warm-up, no equipment, no appointments. Works on any flat surface at home or at the office.' },
          ].map(b => (
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Engineered for Spinal Decompression</h2>
        <div className="pp-desc">
          <p>The Spine Release Back Stretcher is built from firm high-density EVA foam shaped into a precise 26-degree arc that matches the natural lumbar curve of the human spine. Unlike foam rollers that apply pressure unevenly, the structured ridges and acupressure nodules on the surface target the erector spinae and multifidus muscles directly - the two muscle groups most responsible for chronic lower back tension.</p>
          <p>Place it on any flat surface, lie back, and let gravity do the work. The arch gently separates compressed vertebrae, increases space between spinal discs, and triggers a parasympathetic relaxation response in the surrounding musculature. Most users feel a noticeable release within the first 3 minutes.</p>
          <p>The EVA foam construction maintains its shape through years of daily use without softening or compressing. It will not crack, tear, or lose tension. Wipe clean with a damp cloth. Compact enough for a desk drawer or travel bag.</p>
        </div>
        <div className="pp-specs">
          {[
            { label: 'Material',    value: 'High-Density EVA Foam' },
            { label: 'Arc Angle',   value: '26 Degrees' },
            { label: 'Dimensions',  value: '38cm × 28cm × 12cm' },
            { label: 'Weight',      value: '480g' },
            { label: 'Surface',     value: 'Dual Acupressure Nodule Pattern' },
            { label: 'Care',        value: 'Wipe Clean, Waterproof' },
          ].map(s => (
      ...
      <section className="pp-cta-section pp-reveal">
        <div className="pp-cta-label">Ready to recover</div>
        <h2 className="pp-cta-title">Your back has been carrying everything.</h2>
        <p className="pp-cta-sub">Join 1,923 people who gave it the relief it deserves. Free shipping. 30-day guarantee.</p>
        <button className="pp-cta-btn" onClick={() => goToCheckout('Spine Release Back Stretcher', 34.99)}>
          Get Yours Now - $34.99
        </button>
      </section>
      ...
      <div className={`pp-sticky-bar${showStickyBar ? ' pp-sticky-bar--visible' : ''}`} aria-hidden={!showStickyBar}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="pp-sticky-name">Spine Release Back Stretcher</div>
          <div className="pp-sticky-meta">{color.charAt(0).toUpperCase() + color.slice(1)} · $34.99</div>
        </div>
