    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: 14.5, lineHeight: 1.65, color: P.charcoalSoft, textAlign: 'center', margin: '0 0 24px' }}>
          Take our 60-second assessment to learn exactly what screen time is doing to your body and how to reverse it.
        </p>
        <Link
          to="/quiz"
          style={{
...
const PAYMENT_LINK = 'https://buy.stripe.com/9B6aEXggggcj4dSbgydfG00';
...
function ReviewCarousel({ reviews }) {
  ...
  // Track is (reviews.length / visible) × container width wide.
  // translateX as % of track: current / reviews.length × 100
        ...
        {Array.from({ length: maxIndex + 1 }, (_, i) => (
          <button
            ...
            style={{
              ...
              height: 8,
              borderRadius: 4,
              background: i === current ? P.brass : 'rgba(30,27,22,0.16)',
              border: 'none',
              cursor: 'pointer',
              padding: 0,
...
const BASE = import.meta.env.BASE_URL; // '/digital-detox-kit-backend/' per vite.config.js base
...
const COLOR_CONFIG = {
  ivory: {
    ...
    thumbs: [
      ...
      `${BASE}images/ivory-4.png`,
    ],
  },
  midnight: {
    hero: `${BASE}images/midnight-1.png`,
    thumbs: [`${BASE}images/midnight-1.png`],
  },
};
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          boxShadow: '-8px 0 48px rgba(30,27,22,0.14)',
        }}
      >
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: `1px solid ${P.border}`, flexShrink: 0 }}>
          ...
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, color: P.charcoal, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8 }}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11.5, fontFamily: "'IBM Plex Mono', monospace", letterSpacing: '0.08em', color: P.charcoalSoft, textDecoration: 'underline', padding: 0 }}
                  >
                    Remove
                  </button>
                </div>
              </div>
              <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 16, fontWeight: 700, color: P.charcoal, flexShrink: 0, paddingTop: 2 }}>
        ...
        {cart.length > 0 && (
          <div style={{ padding: '20px 24px', borderTop: `1px solid ${P.border}`, flexShrink: 0 }}>
            ...
            <button className="pp-atc" onClick={onCheckout} style={{ margin: 0 }}>
              Checkout
            </button>
          </div>
        )}
...
export default function ProductPage() {
  ...
  const [heroImg, setHeroImg] = useState(COLOR_CONFIG.ivory.hero);
  ...
  useEffect(() => {
    const t = setInterval(() => setViewerCount(Math.floor(Math.random() * 10) + 43), 8000);
    return () => clearInterval(t);
  }, []);
  ...
  const addToCart = () => {
    ...
    const id = `neck-relaxer::${color}`;
    setCart(prev => {
      const existing = prev.find(i => i.id === id);
      if (existing) return prev.map(i => i.id === id ? { ...i, qty: i.qty + qty } : i);
      return [...prev, { id, name: 'Cervical Decompression Neck Relaxer', color: colorLabel, price: 29.99, qty }];
    });
    setCartOpen(true);
  };
  ...
  const LENS_R = 90; // circle radius in px
    ...
    <div style={{ minHeight: '100vh', background: P.linen, color: P.charcoal, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .pp-spec { padding: 18px 0 18px 0; border-bottom: 1px solid ${P.border}; }
        ...
        .pp-ba-box { height: 280px; border: 2px dashed #C8D4C2; border-radius: 16px; background: rgba(238,242,235,0.4); display: flex; align-items: flex-end; justify-content: center; padding-bottom: 16px; }
        .pp-ba-label { font-family: 'IBM Plex Mono', monospace; font-size: 11px; letter-spacing: 0.14em; text-transform: uppercase; color: rgba(58,82,48,0.48); }
        ...
        <div className="pp-nav-right">
          ...
          <button className="pp-cart-icon-btn" onClick={() => setCartOpen(true)} aria-label={`Open cart, ${totalCartItems} item${totalCartItems !== 1 ? 's' : ''}`}>
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
              <path d="M3 8h16l-1.4 9a2 2 0 01-2 1.7H6.4a2 2 0 01-2-1.7L3 8z"/>
              <path d="M8 8V6a3 3 0 116 0v2"/>
            </svg>
      ...
      <section className="pp-hero" ref={heroRef}>
        <div>
          <div
            ...
            style={{ cursor: zoom.active ? 'none' : 'default' }}
          >
            <img
              src={heroImg}
              alt="Cervical Decompression Neck Relaxer"
          ...
          <div className="pp-thumbs">
            {COLOR_CONFIG[color].thumbs.map((src) => (
              <button
                ...
                style={{ outline: heroImg === src ? `2px solid ${P.brass}` : '2px solid transparent' }}
        ...
        <div>
          ...
          <h1 className="pp-headline">
            Cervical Decompression<br />
            <span style={{ fontWeight: 600 }}>Neck Relaxer</span>
          </h1>
          ...
          <div className="pp-swatches">
            {[
              { key: 'ivory', label: 'Ivory', fill: '#F5F0E8' },
              { key: 'midnight', label: 'Midnight', fill: '#1A1A1A' },
            ].map(s => (
              <button
                key={s.key}
                onClick={() => handleColorSelect(s.key)}
                className="pp-swatch-btn"
                aria-label={s.label}
                title={s.label}
                style={{
                  outline: color === s.key ? `2px solid ${P.brass}` : '2px solid transparent',
                  outlineOffset: 2,
                }}
              >
                <span
                  className="pp-swatch-circle"
                  style={{ background: s.fill, border: s.key === 'ivory' ? '1px solid rgba(30,27,22,0.12)' : 'none' }}
                />
          ...
          <div className="pp-stock-row">
            ...
            <span className="pp-stock-text">Only 12 left in stock</span>
          </div>
          ...
          <ul className="pp-bullets">
            ...
            <li className="pp-bullet"><span className="pp-bullet-dot" />10 minutes a day on the floor, couch, or office chair</li>
          </ul>
          ...
          <button className="pp-atc" onClick={addToCart}>
            Add to Cart
          </button>
          <button className="pp-buy-now" onClick={() => goToCheckout('Cervical Decompression Neck Relaxer', 29.99 * qty)}>
            Buy Now
          </button>
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Engineered for the Modern Spine</h2>
        <div className="pp-desc">
          <p>The Cervical Decompression Neck Relaxer is constructed from high-density EVA foam -the same material used in professional orthopaedic supports -shaped into a precise anatomical curve that mirrors the natural lordotic angle of a healthy cervical spine. At 35 degrees forward and 45 degrees inverted, it offers two distinct stretch intensities so you can work at your own pace.</p>
          <p>The surface is covered in thousands of micro-nodules that apply gentle acupressure to the suboccipital muscles at the base of your skull -the exact group that tightens when you look down at a screen for hours. Most users report a noticeable release within the first 4 minutes of use.</p>
          <p>Waterproof and sweat-resistant, it wipes clean in seconds and holds its shape through daily use. Compact enough to fit in a laptop bag, it works on any flat surface -your bedroom floor, office carpet, or the back seat of a car. No straps, no setup, no instructions needed. Just lie down.</p>
        </div>
        <div className="pp-specs">
          {[
            { label: 'Material',       value: 'High-Density EVA Foam' },
            { label: 'Dimensions',     value: '28cm × 22cm × 10cm' },
            { label: 'Weight',         value: '320g' },
            { label: 'Stretch Angles', value: '35° and 45°' },
            { label: 'Surface',        value: 'Micro-Nodule Acupressure' },
      ...
      <section className="pp-cta-section pp-reveal">
        ...
        <button className="pp-cta-btn" onClick={() => goToCheckout('Cervical Decompression Neck Relaxer', 29.99)}>
          Get Yours Now for $29.99
        </button>
      </section>

      <div className={`pp-sticky-bar${showStickyBar ? ' pp-sticky-bar--visible' : ''}`} aria-hidden={!showStickyBar}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="pp-sticky-name">Cervical Decompression Neck Relaxer</div>
      ...
      {zoom.active && zoom.rect && (
        <div
          ...
          style={{
            position: 'fixed',
            left: zoom.clientX - LENS_R,
            top: zoom.clientY - LENS_R,
            width: LENS_R * 2,
            height: LENS_R * 2,
            borderRadius: '50%',
            border: '2px solid #B07D3A',
            boxShadow: '0 4px 20px rgba(30,27,22,0.22)',
            backgroundImage: `url(${heroImg})`,
            backgroundSize: `${zoom.rect.width * ZOOM_FACTOR}px ${zoom.rect.height * ZOOM_FACTOR}px`,
            backgroundPosition: `${-(zoom.x * zoom.rect.width * ZOOM_FACTOR) + LENS_R}px ${-(zoom.y * zoom.rect.height * ZOOM_FACTOR) + LENS_R}px`,
            zIndex: 300,
            pointerEvents: 'none',
            overflow: 'hidden',
          }}
