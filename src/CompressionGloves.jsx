...
const PAYMENT_LINK = 'https://buy.stripe.com/28EbJ17JK2ltcKo2K2dfG09';
...
const REVIEWS = [
  { name: 'Margaret O.', rating: 5, text: 'I have had rheumatoid arthritis for 12 years and these are the first gloves that have actually made a noticeable difference. I wear them every morning and my hands are so much less stiff by midday' },
  { name: 'Kevin L.',    rating: 4, text: 'bought these for my carpal tunnel and they genuinely help. wear them during long coding sessions and the aching is way reduced. only wish they came in a lighter colour too' },
  { name: 'Sandra B.',   rating: 5, text: 'my occupational therapist recommended compression gloves and these are exactly what she described. the copper warmth is real and immediate. i wear them all day at the office now' },
  { name: 'Ravi T.',     rating: 5, text: 'ok so i bought these as a joke gift for my dad who complains about his hands constantly and now he wears them every single day and has ordered two more pairs. so yeah they work' },
  { name: 'Claire M.',   rating: 4, text: 'really good quality and they wash well without losing their shape. i use them while working from home and they have made a big difference to the tingling i was getting in my fingers' },
  { name: 'James P.',    rating: 5, text: 'I am a guitarist and my fretting hand was getting really painful after long sessions. These have extended how long I can play without pain by at least double. Cannot recommend enough' },
  { name: 'Yuki N.',     rating: 5, text: 'these are so much better than i expected?? i have mild carpal tunnel and was skeptical but within the first week the numbness was basically gone. wear them every day now' },
  { name: 'Helen R.',    rating: 4, text: 'good compression and the copper warmth is noticeable. they fit well and stay in place. knocked one star off because the sizing runs slightly small if you have wider hands but otherwise great' },
];
...
const HERO_IMG = `${BASE}images/compressionglove1.png`;
const THUMBS   = ['compressionglove1', 'compressionglove2', 'compressionglove3'].map(n => `${BASE}images/${n}.png`);
...
export default function CompressionGloves() {
  ...
  const [viewerCount, setViewerCount] = useState(53);
  ...
  useEffect(() => {
    const t = setInterval(() => setViewerCount(Math.floor(Math.random() * 20) + 44), 8000);
    return () => clearInterval(t);
  }, []);
  ...
  const addToCart = () => {
    const id = 'compression-gloves';
    setCart(prev => {
      const existing = prev.find(i => i.id === id);
      if (existing) return prev.map(i => i.id === id ? { ...i, qty: i.qty + qty } : i);
      return [...prev, { id, name: 'Copper Compression Therapy Gloves', color: 'One Size', price: 29.99, qty }];
    });
    setCartOpen(true);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: P.linen, color: P.charcoal, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <section className="pp-hero" ref={heroRef}>
        <div>
          <div className="pp-hero-wrap" onMouseMove={handleZoomMove} onMouseLeave={handleZoomLeave} style={{ cursor: zoom.active ? 'none' : 'default' }}>
            <img src={heroImg} alt="Copper Compression Therapy Gloves" className="pp-hero-img" />
          </div>
        ...
        <div>
          ...
          <h1 className="pp-headline">
            ...
            <span style={{ fontWeight: 600 }}>Therapy Gloves</span>
          </h1>
          <StarRow rating={4.6} count={6217} />
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: P.brass, marginBottom: 16 }}>One Size</div>
          ...
          <div className="pp-stock-row">
            ...
            <span className="pp-stock-text">Only 16 left in stock</span>
          </div>
          ...
          <ul className="pp-bullets">
            <li className="pp-bullet"><span className="pp-bullet-dot" />Real copper-infused fabric boosts circulation and eases arthritis, stiffness, and chronic hand pain</li>
            <li className="pp-bullet"><span className="pp-bullet-dot" />Fingerless open-tip design lets you type, scroll, and work without removing the gloves</li>
            <li className="pp-bullet"><span className="pp-bullet-dot" />One size fits most - gentle compression that molds to your hand shape throughout the day</li>
          </ul>
          ...
          <button className="pp-buy-now" onClick={() => goToCheckout('Copper Compression Therapy Gloves', 29.99 * qty)}>Buy Now</button>
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Designed for hands that never stop</h2>
        <div className="pp-benefits-grid">
          {[
            { type: 'copper',    name: 'Real Copper Infusion',         desc: 'Copper-oxide fibres woven into every thread support circulation and reduce joint inflammation for lasting therapeutic relief.' },
            { type: 'dexterity', name: 'Work Without Removing Them',   desc: 'Open fingertips give you full dexterity for typing, scrolling, writing, and everyday tasks while the compression works continuously.' },
            { type: 'wash',      name: 'Machine Washable',             desc: 'Holds compression strength and copper content through hundreds of washes. Just toss them in with your regular laundry.' },
          ].map(b => (
      ...
      <section className="pp-section pp-reveal">
        ...
        <h2 className="pp-section-title">Copper-Infused Relief. All Day Long.</h2>
        <div className="pp-desc">
          <p>The Copper Compression Therapy Gloves are woven from a high-grade nylon and copper-oxide fibre blend that applies consistent graduated compression across the palm, wrist, and finger joints. Copper has been used in therapeutic textiles for decades - it supports circulation, reduces localized inflammation, and creates a mild warmth that soothes arthritic and repetitive strain pain within minutes of wearing.</p>
          <p>The fingerless open-tip design means you never have to take them off to work. Type, scroll, write, knit, or cook while wearing them and receive continuous therapeutic compression throughout. The compression is calibrated to be firm enough to be effective without restricting movement or cutting off circulation.</p>
          <p>The nylon construction is breathable, moisture-wicking, and machine washable. They hold their compression strength through hundreds of washes without losing elasticity. One size fits most adult hands - the fabric stretches and molds to your specific hand shape within the first few wears.</p>
        </div>
        <div className="pp-specs">
          {[
            { label: 'Material',  value: 'Copper-Infused Nylon' },
            { label: 'Copper',    value: 'Real copper-oxide infusion' },
            { label: 'Design',    value: 'Fingerless open-tip' },
            { label: 'Size',      value: 'One size fits most' },
            { label: 'Care',      value: 'Machine wash, air dry' },
            { label: 'Best For',  value: 'Arthritis, RSI, carpal tunnel' },
          ].map(s => (
      ...
      <section className="pp-cta-section pp-reveal">
        ...
        <h2 className="pp-cta-title">Your hands have scrolled, typed, and clicked through thousands of hours.</h2>
        <p className="pp-cta-sub">Give them the support they have earned. Free shipping. 30-day guarantee.</p>
        <button className="pp-cta-btn" onClick={() => goToCheckout('Copper Compression Therapy Gloves', 29.99)}>
          Get Yours Now - $29.99
        </button>
      </section>
      ...
      <div className={`pp-sticky-bar${showStickyBar ? ' pp-sticky-bar--visible' : ''}`} aria-hidden={!showStickyBar}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="pp-sticky-name">Copper Compression Therapy Gloves</div>
          <div className="pp-sticky-meta">One Size · $29.99</div>
        </div>
